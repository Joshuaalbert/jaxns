...
This directory contains the version-isolated benchmark used for the v3 review
gate. `run_v2_standard.py` targets both the PyPI wheel and `main`; the two
sources are measured through different import roots. `run_v3_standard.py`
...
- Seeds: `11,23,37`; MC shrinkage key is deterministically folded from the run
  key and uses 1,000 draws.
- Common settings: v2's default 30 independent Markov chains per dimension,
  ...
  change the v3 race tree. V3 retains `D` early-chain states without treating
  them as depth-loop live points and vmaps 10 replacement chains per
  dimension. The width was selected before the final matrix from an explicit
...
- Scientific release gate: no non-finite runs and no failures of the unchanged
  ...
  `tests/test_ns_standard_problems.py`. The independent three-seed matrix is a
...
conda run -n jaxns_py python \
  ...
  --case spike_slab --seeds 11,23,37 --mc-draws 1000
```

The `main` command substitutes
`PYTHONPATH=/tmp/jaxns_main_2f356d6/src:/home/albert/git/jaxns` and adds
`--implementation-label main --source-id 2f356d6`. The v3 invocation is:

```bash
conda run -n jaxns_py python \
  benchmarks/issue_247/run_v3_standard.py \
  --case spike_slab --seeds 11,23,37 --mc-draws 1000
```
...
conda run -n jaxns_py python benchmarks/issue_247/summarise.py \
  ...
  benchmarks/issue_247/results/final_v3_final/*.jsonl
```
