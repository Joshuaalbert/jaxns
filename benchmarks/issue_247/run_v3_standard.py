"""Issue 247 v3 standard-problem accuracy and performance runner."""
...
from jaxns.results import (
    _block_state_from_results,
    _phantom_group_idx,
    _sample_mc_shrinkage_with_block_state_jit,
)
...
def main():
    ...
    parser.add_argument("--grouping-diagnostics", action="store_true")
    ...
    if args.measure_depth_program:
        ...
        lowered = _run_depth.lower(
            ...
            delta_K=ns.delta_K,
            store_phantom_samples=ns.store_phantom_samples,
    ...
    for seed in [int(value) for value in args.seeds.split(",")]:
        ...
        grouping_records = {}
        if args.grouping_diagnostics:
            block_state = _block_state_from_results(results)
            parent_idx = np.asarray(results.effective_parent_idx)
            root_lineage_idx = np.arange(parent_idx.shape[0], dtype=np.int32)
            for sample_idx, parent in enumerate(parent_idx):
                if 0 <= parent < sample_idx:
                    root_lineage_idx[sample_idx] = root_lineage_idx[parent]
            seed_idx = np.asarray(results.phantom_seed_idx)
            safe_seed_idx = np.clip(seed_idx, 0, seed_idx.shape[0] - 1)
            seed_root_idx = np.where(
                seed_idx >= 0,
                root_lineage_idx[safe_seed_idx],
                np.arange(seed_idx.shape[0], dtype=np.int32),
            )
            grouping_indices = {
                "row": np.arange(int(results.total_num_samples)),
                "seed": seed_idx,
                "root_lineage": seed_root_idx,
            }
            for grouping_name, group_idx in grouping_indices.items():
                grouping_start = time.perf_counter()
                grouped_evidence = _sample_mc_shrinkage_with_block_state_jit(
                    # Hold the MC stream fixed so differences isolate only the
                    # independence grouping convention.
                    key=evidence_key,
                    ...
                    num_Z_samples=args.mc_draws,
                    block_state=block_state,
                    C_min=args.c_min,
                    phantom_group_idx=jnp.asarray(group_idx, dtype=jnp.int32),
                )
                jax.block_until_ready(grouped_evidence)
                grouping_records[f"{grouping_name}_group_log_Z_mean"] = float(
                    grouped_evidence.log_Z_mean
                )
                grouping_records[f"{grouping_name}_group_log_Z_std"] = float(
                    grouped_evidence.log_Z_uncert
                )
                grouping_records[f"{grouping_name}_group_mc_s"] = (
                    time.perf_counter() - grouping_start
                )
        ...
        seed_idx = np.asarray(results.phantom_seed_idx)
        valid_phantom = np.asarray(results.valid_phantom, dtype=bool)
        independence_group_idx = np.asarray(_phantom_group_idx(results))
        valid_seed_idx = seed_idx[valid_phantom & (seed_idx >= 0)]
        if valid_seed_idx.size:
            _, seed_reuse_counts = np.unique(
                valid_seed_idx,
                return_counts=True,
            )
        else:
            seed_reuse_counts = np.zeros((0,), dtype=int)
        record = dict(base)
        record.update({
            "seed": seed,
            "run_s": run_s,
            ...
            "num_reparented": int(state.num_reparented),
            "num_reused_seeds": int(state.num_reused_seeds),
            ...
            "phantom_distinct_seed_groups": int(seed_reuse_counts.size),
            "phantom_independence_groups": int(
                np.unique(independence_group_idx[valid_phantom]).size
            ),
            "phantom_seed_reuse_median": float(
                np.median(seed_reuse_counts)
                if seed_reuse_counts.size
                else 0.0
            ),
            "phantom_seed_reuse_max": int(
                np.max(seed_reuse_counts) if seed_reuse_counts.size else 0
            ),
            "process_peak_rss_kib": resource.getrusage(
                resource.RUSAGE_SELF
            ).ru_maxrss,
        })
        record.update(grouping_records)
