"""Blocks-only control: frozen A with the two phantom lookup calls replaced.

The random stream and eager 64-candidate resolution are copied from
aa79a0dc395c0d64b85cbb88d0e3e732decd7361. This isolates the block-index cost.
"""
import jax
...
from jaxns.sampling.phantom_seeds import seed_stride
from jaxns.state import State
from jaxns.types import BoolArray, FloatArray, IntArray, PRNGKey


def block_eager_stationary_seeds(
        key: PRNGKey,
        state: State,
    ...
    if stride > 1:
        phantom_cumulative_batch = phantom_block_cumulative(
            state.samples, state.num_samples, log_L_constraint, state.phantom_seed_index,
        )  # [S,blocks], only the count/rank implementation differs from reference
    ...
    def select_lane(lane_idx, selected):
        ...
        def rejection_body(carry):
            ...
            if stride > 1:
                phantom_candidate = phantom_block_rank_to_identity(
                    state.samples, state.num_samples, phantom_cumulative, constraint,
                    jnp.clip(
