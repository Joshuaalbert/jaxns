...
def test_mc_linear_evidence_moments_match_deterministic_expectations() -> None:
    ...
    expected = expected_v3_evidence_summary(block_state, concentrations)
    num_draws = 65_536
    sampled = sample_v3_evidence(
...
def _require_callable(name: str):
    fn = getattr(v3_shrinkage, name, None)
    assert callable(fn), f"jaxns.v3_shrinkage.{name} is required by Ticket 0013."
    return fn
...
def test_gamma_weighted_helper_uses_R_cg_not_A_cg_for_open_interval_component() -> None:
...
def test_gamma_weighted_helper_inactive_gate_ignores_phantom_counts_exactly() -> None:
    ...
    block_state = ShrinkageBlockFixture(
        ...
        block_size=np.asarray([1, 1], dtype=np.int32),
