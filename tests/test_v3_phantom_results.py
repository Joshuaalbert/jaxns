...
def test_phantom_gamma_groups_share_root_lineage_identities():
    case = _make_result_case()
    results = dataclasses.replace(
        case.results,
        effective_parent_idx=jnp.asarray([-1, 0, 0], dtype=jnp.int32),
        phantom_seed_idx=jnp.asarray([-1, 0, 1], dtype=jnp.int32),
    )
    np.testing.assert_array_equal(
        np.asarray(results_module._phantom_group_idx(results)),
        np.asarray([0, 0, 0]),
    )
    seed_only = dataclasses.replace(results, effective_parent_idx=None)
    np.testing.assert_array_equal(
        np.asarray(results_module._phantom_group_idx(seed_only)),
        np.asarray([0, 0, 1]),
    )
    ungrouped = dataclasses.replace(results, phantom_seed_idx=None)
    np.testing.assert_array_equal(
        np.asarray(results_module._phantom_group_idx(ungrouped)),
        np.asarray([0, 1, 2]),
    )
...
def _make_padded_plateau_result(num_phantom: int) -> NestedSamplerResults:
    ...
    return NestedSamplerResults(
        ...
        block_size=jnp.asarray([2, 0], dtype=mp_policy.count_dtype),
        block_incoming_K=jnp.asarray([2, 0], dtype=mp_policy.count_dtype),
...
def test_trim_keeps_log_l_blocks_aligned_to_trimmed_sample_size():
    ...
    assert trimmed.log_L_blocks.shape == (2,)
    evidence_samples = trimmed.sample_mc_shrinkage(
        num_samples=4,
        key=jax.random.PRNGKey(13),
    )
    np.testing.assert_allclose(
        np.asarray(trimmed.log_L_blocks),
        np.asarray(evidence_samples.log_L_blocks),
    )
...
def test_resample_keeps_phantom_likelihood_metadata_aligned():
    ...
    assert resampled.log_L_blocks.shape == (1,)
    ...
    np.testing.assert_allclose(resampled.log_L_blocks, resampled.log_L)
    np.testing.assert_allclose(
    ...
    for field_name in (
        ...
        assert getattr(resampled, field_name) is None
...
def test_results_sample_mc_shrinkage_rejects_stale_block_likelihoods_before_jit():
    ...
    results = dataclasses.replace(
        ...
        block_first_idx=jnp.asarray([0, 1], dtype=mp_policy.index_dtype),
        block_size=jnp.asarray([1, 1], dtype=mp_policy.count_dtype),
        block_incoming_K=jnp.asarray([1, 1], dtype=mp_policy.count_dtype),
        block_out_degree=jnp.asarray([0, 0], dtype=mp_policy.count_dtype),
        valid_phantom=jnp.asarray(
            [True, True, False],
            dtype=mp_policy.bool_dtype,
