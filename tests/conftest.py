...
def basic_model_with_obj():
    ...
    def prior_model():
        x = Prior(tfpd.Uniform(low=0, high=1), name='x')
        return log_likelihood(Obj(x))
...
def basic2_model():
    ...
    def prior_model():
        x = Prior(tfpd.Uniform(low=0, high=1))
        return x
...
def basic3_model():
    def prior_model():
        x = Prior(tfpd.Uniform(low=0, high=2), name='x')
        y = Prior(tfpd.Normal(loc=2, scale=x), name='y')
        z = x + y
        return z
