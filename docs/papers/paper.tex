...
\end{align}
Note, when $m_g=1$ this Bayesian model would not make sense because there would be no evidence of an atom, and thus only a 2-class Dirichlet would make sense, which would give us again Eq.~\ref{eq:posterior_pg} as expected.
...
% We need to reconcile the aspect that when m_g=1 a three-class setup is not allowed as previously we saw that the posterior is wrong from the known Beta(K_1, 1). THus we should also take that into account here and not condition on an atom unless we have a reason to suspect that there is a 3rd class. This is pretty important for correctness I think.
