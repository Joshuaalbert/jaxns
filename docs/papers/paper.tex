...
In addition, an auxiliary output of NS is a set of weighted samples drawn from the posterior, with unnormalised weights,
\begin{align}
...
\begin{align}
    ...
    M'_{<g} =& M_{<g} + \mathbf{1}\{C^{\rm Kish}_g \ge C_{\rm min}\} \sum_c v_c A_{cg}\\
    p_g =& \frac{(M'_{>g}, M'_{=g}, M'_{<g})^\top}{M'_{>g} + M'_{=g} + M'_{<g}}.
\end{align}
...
\While{$\hat{n}^i \cdot \hat{n}^0 \ge 0$}
    ...
    \State Reflect $\hat{n}^{i+1} \to \hat{n}^i - 2 (\hat{n}^i \cdot g) g$.
