...
In fact, it's possible to redefine phantom clusters as any disjoint decomposition of phantom samples where each cluster is drawn from within the same parent, and samples from different clusters are independent.
...
\begin{align}
    A_{cg} =&\, \sum_{(x, L) \in \mathcal{P}_c} \mathbf{1}\{L(x) > L_{g-1}\} \mathbf{1}\{L_{p(c)} \le L_{g-1}\}\\
    B_{cg} =&\, \sum_{(x, L) \in \mathcal{P}_c} \mathbf{1}\{L(x) > L_{g}\} \mathbf{1}\{L_{p(c)} \le L_{g-1}\}\\
    E_{cg} =&\, \sum_{(x, L) \in \mathcal{P}_c} \mathbf{1}\{L(x) = L_{g}\} \mathbf{1}\{L_{p(c)} \le L_{g-1}\}\\
    R_{cg} =&\, A_{cg} - B_{cg} - E_{cg}.
\end{align}
Then an MC estimate of $p_g$ is,
\begin{align}
    \hat{p}_{g} =& \frac{\sum_c \left( B_{cg}, E_{cg}, R_{cg}\right)^\top}{\sum_c A_{cg}}.
\end{align}
...
This led to underestimated log-evidence variance in the best of cases.
...
\end{align}
With this choice, singleton clusters would exactly produce the Dirichlet posterior.
...
\begin{align}
    ...
    M'_{<g} =& M_{<g} + \mathbf{1}\{C^{\rm Kish}_g \ge C_{\rm min}\} \sum_c v_c R_{cg}\\
    p_g =& \frac{(M'_{>g}, M'_{=g}, M'_{<g})^\top}{M'_{>g} + M'_{=g} + M'_{<g}}.
\end{align}
...
Phantom-conditioning diagnostics will report the gamma-weighted posterior draws, the per-block Kish participating-cluster count, gate activation, and the resulting change in evidence and shrinkage uncertainty.
