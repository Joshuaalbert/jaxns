...
\title[Phantom-Boosted Nested Sampling]{Phantom-Boosted Nested Sampling}

\author*[1]{\fnm{Joshua G.} \sur{Albert}}\email{albert@strw.leidenuniv.nl}
...
\keywords{Nested Sampling, Bayesian evidence, order statistics, exponential races, Monte Carlo, Markov-chain}
...
The present paper introduces a version of nested sampling in which the volume sequence is treated as a random variable with a race-induced prior that can be conditioned on auxiliary Monte Carlo information.
The auxiliary information comes from \textit{phantom samples}: intermediate states produced by a constrained-prior sampler while generating a classic sample.
...
\begin{itemize}
    ...
    \item Finally, we present JAXNS v3, an open-source implementation of phantom-boosted NS.
\end{itemize}
...
Let $\lambda_0$ be the \textit{sentinel} likelihood associated with $X_{\lambda_0}=1$.
...
We call the subscript $g$ the \textit{block index}.
In addition, an auxiliary output of NS is a set of weighted samples drawn from the posterior, with weights,
\begin{align}
    w_g \triangleq \lambda_g\, (X_{g-1}-X_g),\label{eq:post_weight_cont}
\end{align}
except in the case of samples that fall in likelihood plateaus.
The weights for those will be shown later.
...
\end{align}
and we define a \textit{block} for a level $\lambda_g$ as,
\begin{align}
...
\begin{align}
    r_{p(i) \to i} \sim&\, \mathcal{U}[0,1]\\
    -\log r_{p(i) \to i} \sim&\, \mathrm{Exp}(1).
\end{align}
Define \textit{race time} $s \triangleq -\log X$ and \textit{race rank} $r=\exp -s$, and interpret the race tree as a set of lineages equipped with unit-rate clocks incrementally racing from one sample to the next sample.
...
\begin{align}
    ...
    r_{i-1 \to i} \triangleq& e^{-s_{i-1 \to i}}.\label{eq:beta_shrinkage}
\end{align}
...
This changes the active rate by $-1+d_i$, giving Eq.~\ref{eq:K_recursion}.
\end{proof}

In classic races there are no ties, however in the presence of a plateau, $m_g>1$, we have do have ties.
...
It can easily shown that the final sample in the plateau has known rank,
\begin{align}
    r^{(m_g)}_{g-1 \to g} \sim\, \mathrm{Beta}(K_g - m_g + 1, m_g),\label{eq:end_block_dist}
\end{align}
however since all plateau samples are necessarily inside the atom, we have that $r^{(m_g)}_{g-1 \to g} \neq r_{g-1 \to g}$ almost surely.
...
\begin{figure*}[ht]
    ...
    \caption{The presence of an atom $L=L_g$ causes us to to lose exact knowledge of $p_{>g}$, since $r^{(m_g)}_{g-1 \to g} \in [p_{>g}, p_{>g} + p_{=g}]$. The atom can be seen as an interval in race rank just before finishing the race where samples loose identity.}
    \label{fig:censor}
\end{figure*}
...
\end{align}
where clearly $|p_g| = 1$.
Consider a block with $K_g$ incoming active lineages and $m_g$ block samples.
Then, these define observations,
\begin{itemize}
...
\begin{align}
    p(p_g) = \mathrm{Dir}(1, \epsilon_g, 1 - \epsilon_g),
\end{align}
...
A fairly neutral choice would be, if $m_g=1$ then set $\epsilon_g=10^{-6}$, or else set $\epsilon_g=1/2$ if $m_g>1$.
Using conjugacy of the Dirichlet and multinomial distributions the posterior becomes exactly,
\begin{align}
    &p_g \mid K_g, m_g \sim\, \mathrm{Dir}(a_{>g}, a_{=g}, a_{<g}),\\
    &a_{>g}= K_g - m_g + 1,\\
    &a_{=g}= m_g + \epsilon_g,\\
    &a_{<g}= 1 - \epsilon_g
\end{align}
...
Evidence calculation can be done using,
\begin{align}
    X_g = X_{g-1} p_{>g},
\end{align}
which admits producing samples of the evidence.
Note, when we wish to assign posterior mass to plateau samples in a block this is done by equally assigning atom mass to each sample,
\begin{align}
...
Typically, this is because a Markov chain Monte Carlo (MCMC) approach is used to produce iid samples from $\pi_{\lambda_{p(i)}}$.
Phantom samples are not independent, and thus cannot participate in the race tree, however we are perfectly well allowed to use them to construct unbiased MC estimates of expectations under $\pi_{\lambda_{p(i)}}$.

Let the set of all phantom samples be $\mathcal{P} = \bigcup_i \mathcal{P}_i$ and define counts,
\begin{align}
    A_{g} =&\, \sum_{(x, L) \in \mathcal{P}} \mathbf{1}\{L(x) > L_{g-1}\}\\
    B_{g} =&\, \sum_{(x, L) \in \mathcal{P}} \mathbf{1}\{L(x) > L_{g}\}\mathbf{1}\{L(x) > L_{g-1}\}\\
    E_{g} =&\, \sum_{(x, L) \in \mathcal{P}} \mathbf{1}\{L(x) = L_{g}\} \mathbf{1}\{L(x) > L_{g-1}\}.
\end{align}
then an MC estimate of $p_g$ is,
\begin{align}
    \hat{p}_{g} =& \left(\frac{B_g}{A_g}, \frac{E_g}{A_g}, \frac{A_g - B_g - E_g}{A_g}\right).
\end{align}
Now, if the phantom points were independent draws then the observable $A_g \hat{p}_g$ would satisfy the multinomial likelihood.
However, they are not independent.
Let use propose instead that there are $A^{\rm eff}_{g} = \rho_g A_g$ effective samples, with $0 < \rho_g \le 1$, and that we can use the multinomial likelihood for $A^{\rm eff}_{g} \hat{p}_g$.
Then following the same logic as above we get,
\begin{align}
    &p_g \mid K_g, m_g, \rho_g, \mathcal{P} \sim\, \mathrm{Dir}(b_{>g}, b_{=g}, b_{<g}),\\
    &b_{>g}= K_g - m_g + 1 + \rho_g B_g,\\
    &b_{=g}= m_g + \epsilon_g + \rho_g E_g,\\
    &b_{<g}= 1 - \epsilon_g + \rho_g (A_g - B_g - E_g)
\end{align}
and the posterior marginals for $p_{>g}$ and $p_{=g}$ follow as in Eqs.~\ref{eq:posterior_pg} and \ref{eq:posterior_pe}.

The role of $\rho_g$ is to make the effective multinomial likelihood reproduce the variance of the actual correlated phantom estimator.
Thus, if the phantom samples are iid, then $\rho_g=1$, and for correlated Markov-chain phantom samples, $\rho_g<1$, with inflated multinomial covariance.
We estimate this inflation by bootstrap to take into account the correlation between phantom samples both within and between clusters.
We resample with replacement phantom clusters, compute the empirical covariance of $\hat p_g$, and match it to the analytic iid multinomial covariance.

Since $|p_g|=1$ there are really only two degrees of freedom, so we consider the covariance of the first two components.
Let $q_g=(\hat p_{>g},\hat p_{=g})^\top$, and let $\widehat\Sigma_g^{\rm boot}$ be the covariance of $q_g$ under cluster bootstrap.
The analytic model covariance is,
\begin{align}
\widehat\Sigma_g =
\frac{1}{A_g}\begin{pmatrix}
\hat p_{>g}(1-\hat p_{>g}) & -\hat p_{>g}\hat p_{=g}\\
-\hat p_{>g}\hat p_{=g} & \hat p_{=g}(1-\hat p_{=g})
\end{pmatrix}.
\end{align}
The iid covariance would be $\widehat\Sigma_g$, while the effective-count model predicts $\widehat\Sigma_g/\rho_g$.
Thus using that $\mathrm{tr}(\mathbf{A}^+\mathbf{A}) = \mathrm{rank}(\mathbf{A})$ we propose an estimator,
\begin{align}
\hat\rho_g
=
\frac{\mathrm{rank}(\widehat\Sigma_g)}
{\mathrm{tr}\left(\widehat\Sigma_g^{+}\widehat\Sigma_g^{\rm boot}\right)},
\end{align}
where ${}^+$ means Moore-Penrose inverse.
Since individual $\hat\rho_g$ values are noisy, a better approach is to fit a low-order function approximator against normalised race time,
\begin{align}
    \rho_g=c_0 + c_1\frac{s_g}{s_G} + c_2\left(\frac{s_g}{s_G}\right)^2.
\end{align}
...
The above formulation cannot correct for non-stationary phantom samples, thus a burn-in is suggested before collecting phantoms.
...
The race tree permits new children to be generated from within any existing sample contour.
This decouples correct lineage counting from the strategy of sample generation, and allows us to explore optimal allocation targets as an experimental design question.
Algorithm~\ref{algo:jaxns} shows the core algorithm of JAXNS.
There is an outer loop which sets a lineage allocation target as a function of likelihood, $K^k_*:\mathbb{R}_+ \to \mathbb{N}$.
There is an inner loop which, given $K^k_*$, generates samples until a depth condition is met.
Sample generation can be embarrassingly parallelised.
The outer loop proceeds until a goal condition is met.
During the inner loop samples are identified whose active lineage counts do not meet the allocation target, and a sampling contour is chosen with high probability of increasing the active lineage count.
A strong way to choose a parent block with a high probability to reach block $g'$ is choose block $g$ with probability $(X_{g'}/X_g) \,\mathbf{1}\{X_{g'} < X_g\}$.
The depth condition captures when there is little utility in continuing with the current allocation target, $K^k_*$.
...
\begin{align}
    ...
    \text{small remaining posterior mass},\qquad& \frac{L_gX_g}{\max_{g'}(L_{g'}X_{g'})} < \tau_{\rm post}
\end{align}
The goal condition is user specified, and can be any condition computable from the samples, for example until a target effective sample size or evidence uncertainty is met.
...
\begin{align}
    \text{Depth-first uniform}, \qquad& K^k_*(L) = d_0 + k  \Delta K,\\
...
\While{$\neg\, \mathbf{goal}$}
    \State Choose allocation target function $K^{k}_*$.
    \While{$\neg\, \mathbf{depth}$}
        \State Count lineages $K_i$ for $i\in\{1 \ldots N\}$.
...
\Statex \textbf{Note 2:} We do not need to keep track of parents for each sample, since the algorithm only needs to know out-degree. If parent blocks were desired, they can be uniquely reconstructed from sorted likelihoods and out-degrees.
\end{algorithmic}
\label{algo:jaxns}
\end{algorithm}
...
In choosing $\mathcal{D}$, JAXNS makes available two choices: an isotropic Gaussian, and an ellipsoidal Gaussian.
...
Concerning the construction of the trajectory, and sampling from it, there are two available methods in JAXNS.
The first is perfect bracketing along a straight line, where the endpoints are chosen to be intersection with the bounds of the unit-hypercube representing the prior homogeneous measure.
...
This approach trades off the zero-cost interval selection with extra likelihood evaluations to shrink the slice.
...
The trajectory is constructed a from the seed point $x^i$ in both directions $\hat{n}$ and $-\hat{n}$ until the momentum vector is anti-aligned with the initial direction, a so-called \textit{U-turn}.
Algorithm~\ref{algo:galilean} describes creating a single side of the trajectory from a seed point in one direction.
The full trajectory is generally non-straight, that bounces off the hard boundary constraints.
Since the trajectory \textit{should} fall within the slice, uniform sampling without trajectory shrinkage is used.
...
\begin{algorithm}
\caption{Galilean single-side trajectory building by bracketing the contour endpoint and Hausdorff reflecting from the inner point until a U-turn.}
\begin{algorithmic}[1]
...
\While{$\hat{n}^i \cdot \hat{n}^0 \ge 0$}
    ...
    \If{$L(x') \le \lambda$} \hfill\textit{Backtrack into contour}
        ...
        \State Reflect $\hat{n}^{i+1} \to \hat{n}^i - 2(\hat{n}^i \cdot \nabla L(x')) \nabla L(x')$.
        \State Normalise $\hat{n}^{i+1} \to \hat{n}^{i+1}/|\hat{n}^{i+1}|$.
        \State Set $x^{i+1} \to x'$.
        \State Set $i \to i+1$.
    \Else \hfill\textit{Step out of contour}
        % Step further.
        \State Set $\Delta t \to 2 \Delta t$.
        \State Set temporary $x'' \to x'$
        \State Propose step forward $x' \to x^i + \Delta t \hat{n}^i$
        \While{$L(x') > \lambda$}
...
Workers receive serialized likelihood models and are scheduled by load balancing.
...
The race-tree formulation is key to this asynchronous setting because a completed child only updates the out-degree of its parent which is known before dispatching sample generation.




\section{Experimental protocol}\label{sec:experiments}
{\color{red} Josh: The empirical results in this version are placeholders for new JAXNS benchmarks.}
...
This paper reformulates NS shrinkage as race lineages, which decouples the selection of contours from the sample generation process.
We then introduce a rigorous Bayesian model for dealing with plateaus and phantom samples.
