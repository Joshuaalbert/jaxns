"""Run the frozen A+C240 suite on sixty distinct CPU cores."""
import hashlib
import json
import os
...
VARIANTS = ('AC240',)
CASES = ('basic_mvn', 'weak_curved_mvn8', 'spike_slab', 'curved_spike_slab8')
ROOTS = {v: Path('/largedata/albert/git') / f'jaxns-paper-{v}' for v in VARIANTS}
...
(OUT / 'FINISHED.json').write_text(json.dumps(dict(complete=True, cells=120, max_parallel=60, cpus=CPUS, elapsed_seconds=time.time()-started), indent=2))
