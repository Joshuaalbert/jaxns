"""Adopt live SS10 workers and schedule against per-tree memory estimates."""
...
for stage in ("0.02", "0.01", "0.005"):
    ...
    while pending or active:
        ...
        if not active and pending:
            raise RuntimeError(
                "Pending work cannot be dispatched; inspect memory/disk/dependencies"
            )
        if active:
            time.sleep(2)
    if stage_failures:
