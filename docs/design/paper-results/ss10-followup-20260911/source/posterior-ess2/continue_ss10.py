"""Continue SS10 under evidence allocation or posterior allocation to an ESS target.

Evidence continuation converts the old row cache once. Posterior continuation
starts from the efficient 0.015 state and fixes its ESS target from that input,
including on checkpoint resume. Both keep all phantom U points and input keys.
...
def main() -> None:
    ...
    stopping.add_argument("--target", type=float)
    stopping.add_argument("--posterior-ess-multiplier", type=float)
    parser.add_argument("--checkpoint-every", type=int, default=10)
    parser.add_argument("--resume", action="store_true")
    args = parser.parse_args()
    posterior = args.posterior_ess_multiplier is not None
    if not 0 <= args.seed < 30:
        parser.error("Use seeds 0--29")
    if posterior:
        if not np.isfinite(args.posterior_ess_multiplier) or args.posterior_ess_multiplier <= 1:
            parser.error("The posterior ESS multiplier must be finite and greater than one")
    else:
        args.target = .015 if args.target is None else args.target
        if not 0 < args.target < .02:
            parser.error("Use a positive uncertainty below 0.02")
    ...
    initial_ess = None
    ess_target = None
    allocation_target = "posterior_improving" if posterior else "evidence_improving"
    if posterior:
        assert previous["target"] == .015 and provenance["source_commit"].startswith("82cd8d3")
        initial_ess = previous["accuracy"]["kish_ess"]
        ess_target = args.posterior_ess_multiplier * initial_ess
    else:
        assert provenance["goal_classic_log_Z_uncert"] == .02
    ...
    manifest = dict(
        ...
        allocation_target=allocation_target, initial_ess=initial_ess, ess_target=ess_target,
        posterior_ess_multiplier=args.posterior_ess_multiplier,
        ...
        sampler="efficient all-phantom seeds, isotropic, d0=delta_K=300, "
                "shell=100, slices=100, P=99",
    )
    if posterior:
        assert manifest["source_sha256"] == provenance["source_sha256"]
    ...
    if not args.resume and not posterior:
    ...
    if posterior:
        assert state.phantom_seed_index is not None and bool(state.depth_reached)
        if not args.resume:
            # Freeze the requested doubling against the actual loaded classic tree.
            assert float(classic_metrics(state).kish_ess) == initial_ess
    gc.collect()
    core = NestedSampler(
        ...
        allocation_target=allocation_target, unlimited_samples=True,
    ...
    reference_rows = {}
    if not posterior and old_progress.exists():
    ...
    def goal(value: State) -> bool:
        ...
        success = (float(metrics.kish_ess) >= ess_target if posterior
                   else float(metrics.log_Z_uncert) < args.target)
        ...
        row = dict(
            ...
            ess_ratio=float(metrics.kish_ess) / initial_ess if posterior else None,
    ...
    record = dict(
        ...
        allocation_target=allocation_target, initial_ess=initial_ess, ess_target=ess_target,
        ess_ratio=accuracy["kish_ess"] / initial_ess if posterior else None,
