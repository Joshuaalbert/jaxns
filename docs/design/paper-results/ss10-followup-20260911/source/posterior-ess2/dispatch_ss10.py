...
The final summary compares classic accuracy with the selected input cohort.
"""
...
parser.add_argument("--posterior-ess-multiplier", type=float)
...
if posterior:
    if not np.isfinite(args.posterior_ess_multiplier) or args.posterior_ess_multiplier <= 1:
        parser.error("The ESS multiplier must be finite and greater than one")
    archive = root / "benchmarks/phantom_seeding/results/ss10-0015-20260910"
else:
...
reservations = (
    {str(row["seed"]): row["memory_gib"] for row in forecast} if posterior
    else {str(seed): 80. if seed in large_seeds else 44. for seed in range(30)}
)
assert all(0 < value <= args.memory_gib for value in reservations.values())
initial_label = "0.015" if posterior else "0.02"
final_label = f"posterior_ess_x{args.posterior_ess_multiplier:g}" if posterior else "0.015"
manifest = dict(
    target=None if posterior else .015, initial_target=.015 if posterior else .02,
    seeds=list(range(30)), source_commit=source_commit,
    ...
    reservation_gib=reservations, posterior_ess_multiplier=args.posterior_ess_multiplier,
...
while pending or active:
    ...
    for prediction in list(pending):
        ...
        command = [
            ...
            "python", str(runner), "--seed", str(seed),
            "--input", str(archive / f"seed-{seed:02d}"), "--output", str(cell),
        ]
        if posterior:
            command.extend(["--posterior-ess-multiplier", str(args.posterior_ess_multiplier)])
        else:
            command.extend(["--target", ".015"])
...
for seed in range(30):
    ...
    if posterior:
        assert result["allocation_target"] == "posterior_improving"
        assert result["initial_ess"] == baseline["accuracy"]["kish_ess"]
        assert result["ess_target"] == args.posterior_ess_multiplier * result["initial_ess"]
        assert result["accuracy"]["kish_ess"] >= result["ess_target"]
        baseline_accuracy = baseline["accuracy"]
    else:
        baseline_analysis = json.loads((archive / f"seed-{seed:02d}/ANALYSIS.json").read_text())
        assert result["classic_log_Z_uncert"] < .015
    ...
    records.append(dict(seed=seed, result=result, baseline=baseline_accuracy))
summary = dict(complete=True, seeds=30, elapsed_seconds=time.time() - started, targets={})
for target in (initial_label, final_label):
    values = [row["baseline"] if target == initial_label else row["result"]["accuracy"]
              for row in records]
