...
- Naming: modules `snake_case.py`; functions/vars `snake_case`; classes `PascalCase`; consts `UPPER_SNAKE_CASE`; private
  `_name`.
- Structure: production code in `src/jaxns/...`; tests in `tests/...`; cross-package imports
  allowed, but beware circular, `bx_common` is used for shared deps. Don't put anything inside `__init__.py` files.
