...
from jaxns.internals.tree_utils import tree_dot
from jaxns.nested_samplers.mixed_precision import mp_policy
from jaxns.nested_samplers.pytree import PureDataclassPytree
from jaxns.nested_samplers.random_utils import sample_uniformly_masked
from jaxns.nested_samplers.samples import UType, Samples
from jaxns.nested_samplers.types import FloatArray, IntArray, PRNGKey, UType, BoolArray, SampleCollection, Sample, SeedPoint
...
class AbstractSampler(ABC):
    ...
    @abstractmethod
    def get_sample(self, key, log_L_constraint: FloatArray, seed_point: SeedPoint) -> tuple[UType, FloatArray, IntArray]:
        """
...
class UniDimSliceSampler(AbstractSampler, PureDataclassPytree):
    ...
    def flatten(cls, this) -> tuple[list[Any], tuple[Any, ...]]:
        return cls.build_flatten(this, ['num_slices', 'num_phantom_save', 'midpoint_shrink', 'perfect', 'gradient_slice', 'adaptive_shrink', 'gradient_guided'])
