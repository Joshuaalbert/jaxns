...
class State(PureDataclassPytree):
    ...
    def flatten(cls, this) -> tuple[list, tuple]:
        return cls.build_flatten(
            this,
            ["model", "args", "params", "execution_diagnostics"],
        )
    ...
    def to_result(self: NestedSamplerResults) -> NestedSamplerResults:
        ...
        """
        _validate_evidence_block_capacity(self)
        state_without_diagnostics = dataclasses.replace(
            self,
            execution_diagnostics=None,
        )
        result = _to_result(state_without_diagnostics)
        if self.execution_diagnostics is None:
            return result
        return attach_execution_diagnostics(result, self.execution_diagnostics)
...
def _to_result(self: State) -> NestedSamplerResults:
    ...
    num_live_points_per_sample = self.samples.compute_num_live_points_per_sample(
        root_out_degree=self.root_out_degree,
        num_samples=self.num_samples,
    )
