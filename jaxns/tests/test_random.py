...
def test_random_ortho_matrix():
    ...
    assert jnp.allclose(jaxns.common.T @ M, M @ jaxns.common.T, atol=1e-6)
    assert jnp.allclose(jaxns.common.T @ M, jnp.eye(5), atol=1e-6)
...
def test_random_ortho_normal_matrix():
    for i in range(100):
        ...
        assert jnp.all(jnp.isclose(H @ jaxns.common.T, jnp.eye(3), atol=1e-6))
